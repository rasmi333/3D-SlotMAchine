...
public class OpenGLRenderer implements Renderer 
{
	...
	public OpenGLRenderer(Context context, Handler handler) 
	{
		...
		spinner.loadBitmap(BitmapFactory.decodeResource(context.getResources(), R.drawable.slot6_pics));
...
//		Log.i(DEBUG_TAG, "onsurfaceChanged()");
						...
						mHandler.post(new Runnable() {
							...
							//	toastLayout.addView(text);   
				    			...
				    			// ADD CODE HERE
								 ...
								 note.setLatestEventInfo(mContext, "You hit big!!", "You are the jackpot winner!!", pendingIntent);
	...
	public boolean doTouchEvent (MotionEvent event) 
    {
		...
		if (event.getAction() == MotionEvent.ACTION_DOWN) {
			...
			float width=((Activity)mContext).getWindow().getWindowManager().getDefaultDisplay().getWidth();
			float height=((Activity)mContext).getWindow().getWindowManager().getDefaultDisplay().getHeight();
			float x1=0.133f*width;
			float x2=width-x1;
			float y1=height-0.31f*height;
			float y2=height-0.16f*height;
			Log.v("x ","x1="+x1+"    y1="+y1+"     x2="+x2+"       y2="+y2);
